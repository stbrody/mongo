...
TEST(LockerImpl, AtomicLockRestoreAndGlobalLockReleaseForReplStateTransitions) {
    ...
    // Take some locks, mimicing the locks that would be held by ongoing prepared transactions.
    ...
    // Enqueue request for global X lock in stepUpLocker, mimicing the lock held by the thread
    ...
    ASSERT_EQUALS(MODE_IS, randomOpLocker2.getLockMode(globalResId));
}
}  // namespace mongo
