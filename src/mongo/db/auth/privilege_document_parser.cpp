...
namespace {
    ...
    Status _checkV2RolesArray(const BSONElement& rolesElement) {
        if (rolesElement.type() != Array) {
            return _badValue("Role fields must be an array", 0);
        }
        for (BSONObjIterator iter(rolesElement.embeddedObject()); iter.more(); iter.next()) {
            if ((*iter).type() != Object) {
    ...
    Status V2PrivilegeDocumentParser::checkValidPrivilegeDocument(const StringData& dbname,
        ...
        if (delegatableRolesElement.eoo()) {
            return _badValue("User document needs 'delegatableRoles' field to be provided", 0);
        }
        status = _checkV1RolesArray(delegatableRolesElement);
        if (!status.isOK())
            return status;
