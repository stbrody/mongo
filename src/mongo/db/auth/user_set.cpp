...
namespace mongo {
    ...
    UserSet::UserSet(AuthorizationManager* authzManager) : _authzManager(authzManager) {}
    UserSet::~UserSet() {
    ...
    void UserSet::removeByDBName(const StringData& dbname) {
        for (std::vector<User*>::iterator it = _users.begin();
                it != _users.end(); ++it) {
            User* current = *it;
