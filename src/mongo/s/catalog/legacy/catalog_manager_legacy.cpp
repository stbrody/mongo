...
Status CatalogManagerLegacy::shardCollection(OperationContext* txn,
    ...
    auto ssvStatus = grid.shardRegistry()->runIdempotentCommandOnShard(
        txn, dbPrimaryShardId, ReadPreferenceSetting{ReadPreference::PrimaryOnly}, "admin", ssv.toBSON());
    if (!ssvStatus.isOK()) {
...
Status CatalogManagerLegacy::dropCollection(OperationContext* txn, const NamespaceString& ns) {
    ...
    for (const auto& shardEntry : allShards) {
        auto dropResult = shardRegistry->runIdempotentCommandOnShard(
            txn, shardEntry.getName(), ReadPreferenceSetting{ReadPreference::PrimaryOnly}, ns.db().toString(), BSON("drop" << ns.coll()));
    ...
    for (const auto& shardEntry : allShards) {
        ...
        auto ssvResult = shardRegistry->runIdempotentCommandOnShard(
            txn, shardEntry.getName(), ReadPreferenceSetting{ReadPreference::PrimaryOnly}, "admin", ssv.toBSON());
        ...
        auto unsetShardingStatus = shardRegistry->runIdempotentCommandOnShard(
            txn, shardEntry.getName(), ReadPreferenceSetting{ReadPreference::PrimaryOnly}, "admin", BSON("unsetSharding" << 1));
