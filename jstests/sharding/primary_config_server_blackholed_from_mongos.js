...
(function() {
    ...
    for (var i = 0; i < 1000; i++) {
        assert.writeOK(testDB.ShardedColl.insert({_id: i}));
    }

    jsTest.log('Partitioning the congig server primary from the mongos');
    ...
    assert.throws(function() {configDB.chunks.find().itcount(); });
    assert.throws(function() {configDB.chunks.find().count(); });
//    assert.throws(function() {configDB.chunks.aggregate().count(); });
